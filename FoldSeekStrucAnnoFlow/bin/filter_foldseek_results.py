# Adapted from format_fs_output.py in the domain annotation pipeline, with updated thresholds and output format. Also added qcov and tcov to the output for potential future use in filtering.

from pathlib import Path
from typing import Annotated, TextIO

import typer

# Threshold constants
H_EVALUE_THRESHOLD: float = 0.061144  # previous: 0.019000
H_COVERAGE_THRESHOLD: float = 0.459063  # previous: 0.366757
H_TMSCORE_THRESHOLD: float = 0.634640  # previous: 0.560000
T_EVALUE_THRESHOLD: float = 0.476641  # previous: 0.108662
T_COVERAGE_THRESHOLD: float = 0.741603  # previous: 0.786333
T_TMSCORE_THRESHOLD: float = 0.513274  # previous: 0.416331

OUTPUT_COLUMNS: list[str] = [
    "query_id",
    "target_id",
    "evalue",
    "tmscore",
    "type",
    "qcov",
    "tcov",
]
WRITTEN_HEADERS: bool = False


app = typer.Typer()


@app.command()
def main(
    input_file: Annotated[Path, "-i", "--input", "Path to the Foldseek output file."],
    output_file: Annotated[Path, "-o", "--output", "File to write the parsed results."],
) -> None:
    with open(input_file, "r") as infile, open(output_file, "w") as outfile:
        current_query_hits: list[dict] = []
        current_query_id: str = ""

        for line in infile:
            recs = line.strip().split()
            if len(recs) < 8:
                continue

            query_id, target_id, evalue, qlen, tlen, qtmscore, ttmscore, qcov, tcov = parse_record(recs)
            if current_query_id == "":
                current_query_id = query_id
            if query_id != current_query_id:
                best_hit = determine_best_hit(current_query_hits)
                if best_hit:
                    write_best_hit(outfile, current_query_id, best_hit)
                current_query_hits = []
                current_query_id = query_id

            update_hits(current_query_hits, target_id, evalue, qlen, tlen, qtmscore, ttmscore, qcov, tcov)
        if current_query_hits:
            best_hit = determine_best_hit(current_query_hits)
            if best_hit:
                write_best_hit(outfile, current_query_id, best_hit)


def parse_record(record: list[str]) -> tuple[str, str, float, int, int, float, float, float, float]:
    if len(record) != 10:
        raise ValueError(f"failed to get expected columns in foldseek output (got {len(record)} expected 10)")
    query_id, target_id = record[0], record[1]
    evalue, qlen, tlen, qtmscore, ttmscore, qcov, tcov = map(float, record[3:])
    return query_id, target_id, evalue, int(qlen), int(tlen), qtmscore, ttmscore, qcov, tcov


def update_hits(
    current_query_hits: list[dict],
    target_id: str,
    evalue: float,
    qlen: int,
    tlen: int,
    qtmscore: float,
    ttmscore: float,
    qcov: float,
    tcov: float,
) -> None:
    coverage = min(qlen, tlen) / max(qlen, tlen)
    tmscore = max(qtmscore, ttmscore)
    hit_type = determine_hit_type(evalue, coverage, tmscore)

    current_query_hits.append(
        {
            # "code": code,
            "target_id": target_id,
            "evalue": evalue,
            "coverage": coverage,
            "tmscore": tmscore,
            "type": hit_type,
            "qcov": qcov,
            "tcov": tcov,
        }
    )


def determine_hit_type(evalue: float, coverage: float, tmscore: float) -> str | None:
    if evalue < H_EVALUE_THRESHOLD and coverage > H_COVERAGE_THRESHOLD and tmscore > H_TMSCORE_THRESHOLD:
        return "H"
    elif evalue < T_EVALUE_THRESHOLD and coverage > T_COVERAGE_THRESHOLD and tmscore > T_TMSCORE_THRESHOLD:
        return "T"
    else:
        if tmscore > T_TMSCORE_THRESHOLD:
            return "N"
        else:
            return None


def determine_best_hit(hits: list[dict]) -> dict | None:
    h_hits = [hit for hit in hits if hit["type"] == "H"]
    t_hits = [hit for hit in hits if hit["type"] == "T"]
    n_hits = [hit for hit in hits if hit["type"] == "N"]
    no_hits = [hit for hit in hits if hit["type"] == "No_Hit"]

    h_hits.sort(key=lambda x: (x["evalue"], -x["tmscore"]))
    t_hits.sort(key=lambda x: (-x["tmscore"], x["evalue"]))

    if h_hits:
        return h_hits[0]
    elif t_hits:
        return t_hits[0]
    elif n_hits:
        n_hits.sort(key=lambda x: (-x["tmscore"], x["evalue"]))
        return n_hits[0]
    elif no_hits:
        no_hits.sort(key=lambda x: (-x["tmscore"], x["evalue"]))
        return no_hits[0]
    return None


def write_best_hit(outfile: TextIO, query_id: str, best_hit: dict) -> None:
    global WRITTEN_HEADERS
    if WRITTEN_HEADERS is False:
        outfile.write("\t".join(OUTPUT_COLUMNS) + "\n")
        WRITTEN_HEADERS = True
    best_hit["query_id"] = query_id
    outfile.write("\t".join([str(best_hit[colname]) for colname in OUTPUT_COLUMNS]) + "\n")


if __name__ == "__main__":
    app()
