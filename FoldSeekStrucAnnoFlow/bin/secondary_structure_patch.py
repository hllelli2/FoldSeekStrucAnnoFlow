...
def calculate_ss(pdbfile, chain, stride_path, ssfile="pdb_ss"):
    ...
    with open(ssfile, "w") as ssout_file:
        ...
        try:
            subprocess.run(args, stdout=ssout_file, stderr=subprocess.DEVNULL, check=True)
...
def make_ss_matrix(ss_path, nres):
    ...
    with open(ss_path, errors="replace") as f:
        lines = f.readlines()
    type_set = set()
...
def renum_pdb_file(pdb_path, output_pdb_path):
    ...
    with open(output_pdb_path, "w") as output_file:
        subprocess.run(["python", str(pdb_reres_path), pdb_path], stdout=output_file, check=True, text=True)
